<template>
  <section @keyup.ctrl.90="undoMerge">
    <!-- Ingredient Link Editor, opens on step click -->
    <v-dialog v-if="dialog" v-model="dialog" width="600">
      <v-card :ripple="false">
        ...
        <v-card-text class="pt-4">
          ...
          <!-- the ingredient reference choosing list -->
          <v-checkbox v-for="ing in unusedIngredients" :key="ing.referenceId" v-model="activeRefs"
            :value="ing.referenceId" class="mb-n2 mt-n2">
            <template #label>
          ...
          <template v-if="usedIngredients.length > 0">
            ...
            <v-checkbox v-for="ing in usedIngredients" :key="ing.referenceId" v-model="activeRefs"
              :value="ing.referenceId" class="mb-n2 mt-n2">
              <template #label>
                <RecipeIngredientHtml :markup="parseIngredientText(ing, recipe.settings.disableAmount)" />
              </template>
        ...
        <v-card-actions>
          ...
          <div class="d-flex flex-wrap justify-end">
            ...
            <!-- The auto ingredients reference setter button  -->
    ...
    <div class="d-flex justify-space-between justify-start">
      ...
      <BaseButton v-if="isEditForm" @click="initStepsWithReferences">
        <template #icon>
          {{ $globals.icons.robot }}
        </template>
        Auto Reference ingredients
      </BaseButton>
    </div>
          ...
          <v-hover v-slot="{ hover }">
            <v-card class="my-3" :class="[{ 'on-hover': hover }, isChecked(index)]" :elevation="hover ? 12 : 2"
              ...
              <v-card-title :class="{ 'pb-0': !isChecked(index) }">
                ...
                <!-- auto attach ingredient reference to  -->
              ...
              <DropZone @drop="(f) => handleImageDrop(index, f)">
                <v-card-text v-if="isEditForm" @click="$emit('click-instruction-field', `${index}.text`)">
                  ...
                  <RecipeIngredientHtml v-for="ing in step.ingredientReferences" :key="ing.referenceId"
              ...
              <v-expand-transition>
                <div v-show="!isChecked(index) && !isEditForm" class="m-0 p-0">
                  ...
                  <v-card-text class="markdown">
                    <v-row>
                      <v-col v-if="isCookMode && step.ingredientReferences && step.ingredientReferences.length > 0"
                        cols="12" sm="5">
                        <div class="ml-n4">
                          ...
                          })" :scale="scale" :disable-amount="recipe.settings.disableAmount"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    // use to auto reference all ingredients for each step
    function initStepsWithReferences() {
      for (const [idx, step] of props.value.entries()) {
        console.log(idx, step);

        let refs = step.ingredientReferences;
        if (!refs) {
          props.value[idx].ingredientReferences = [];
          refs = props.value[idx].ingredientReferences as IngredientReferences[];
        }
